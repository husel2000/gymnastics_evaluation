...
if(empty($_POST['action'])) {
	...
	$res = db_select("Select datum,bezeichnung,typ,jahrgang_min, jahrgang_max, opt_text1, opt_text2, id_wettkampf from wettkampf where id_wettkampf = ?",$id_wettkampf);
	$data = Array("datum" => $res[0][0], "bezeichnung" => $res[0][1], "typ" => $res[0][2], "jahrgang_min" => $res[0][3], "jahrgang_max" => $res[0][4], "opt_text1" => $res[0][5], "opt_text2" => $res[0][6], "id_wettkampf" => $res[0][7]);
	...
	if(empty($id_wettkampf)) {
		...
		$id_wettkampf = db_select("INSERT INTO wettkampf (datum,bezeichnung,jahrgang_min,jahrgang_max,typ) VALUES(STR_TO_DATE(?,'%d.%m.%Y'),?,?,?,?)",$datum,$name,$jahrgang_min,$jahrgang_max,$typ);
		...
		db_select("UPDATE wettkampf SET datum = STR_TO_DATE(?,'%d.%m.%Y'), bezeichnung = ?, jahrgang_min = ?, jahrgang_max = ?
			WHERE id_wettkampf = ?",$datum,$name,$jahrgang_min, $jahrgang_max,$id_wettkampf);
	}
